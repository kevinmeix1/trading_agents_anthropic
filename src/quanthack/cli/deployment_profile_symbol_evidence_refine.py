...
from quanthack.backtesting.deployment_profile_symbol_evidence_refiner import (
    DEFAULT_PROBE_MULTIPLIERS,
    DEFAULT_STALE_AFTER_BARS,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep targeted online symbol-evidence gates for fragile deployment-profile "
            "symbols, including reduced no-history probe sizing."
        )
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Symbol to gate. Repeat to override the fragile symbols from robustness.",
    )
    parser.add_argument(
        "--probe-multiplier",
        action="append",
        type=float,
        default=None,
        help="No-history probe target multiplier. Repeat to override defaults.",
    )
    parser.add_argument(
        "--stale-after-bars",
        action="append",
        type=int,
        default=None,
        help="Bars after which closed evidence becomes stale. Use 0 for never stale.",
    )
    ...
    parser.add_argument(
        "--output",
        default="outputs/research/deployment_profile_symbol_evidence_gate.csv",
    )
    parser.add_argument(
        "--recommendation-json",
        default="outputs/research/deployment_profile_symbol_evidence_gate.json",
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    result = refine_deployment_profile_symbol_evidence_gate(
        ...
        probe_multipliers=tuple(args.probe_multiplier or DEFAULT_PROBE_MULTIPLIERS),
        stale_after_bars_values=tuple(args.stale_after_bars or DEFAULT_STALE_AFTER_BARS),
    ...
    write_deployment_profile_symbol_evidence_gate_csv(result, args.output)
    write_deployment_profile_symbol_evidence_gate_json(result, args.recommendation_json)

    print("Deployment Profile Symbol Evidence Gate")
    ...
    print(f"  Recommendation JSON: {args.recommendation_json}")
    for rank, candidate in enumerate(result.candidates[: max(args.limit, 0)], start=1):
        ...
        print(
            ...
            f"targets={candidate.target_symbols_text or 'all'}, "
            f"probe={spec.no_history_target_multiplier:.2f}, "
            f"stale={spec.stale_after_bars or 'never'}, "
