...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "symbol-specific UTC hours found in session attribution."
        )
    )
    ...
    parser.add_argument("--max-dropped-symbol-hours", type=int, default=5)
    ...
    parser.add_argument(
        "--output",
        default="outputs/research/deployment_profile_symbol_gate_refinement.csv",
    )
    parser.add_argument(
        "--refined-pack-json",
        default="outputs/research/deployment_profile_symbol_gated_pack.json",
    )
    parser.add_argument("--refined-slot", default="symbol_refined")
...
def run(args: argparse.Namespace) -> None:
    ...
    result = refine_deployment_profile_symbol_gates(
        ...
        max_dropped_symbol_hours=args.max_dropped_symbol_hours,
    ...
    write_deployment_profile_symbol_gate_refinement_csv(result, args.output)
    ...
    print("Deployment Profile Symbol-Gate Refinement")
    print(f"  Base slot: {result.base_profile.slot}")
    print(f"  Base label: {result.base_profile.label}")
    print(
        "  Weak symbol hours: "
        + (
            ", ".join(f"{row.symbol}:{row.utc_hour}" for row in result.weak_symbol_hours)
            or "none"
        )
    )
    ...
    for rank, candidate in enumerate(result.candidates[: max(args.limit, 0)], start=1):
        ...
        print(
            ...
            f"dropped={candidate.dropped_symbol_hours_text or 'none'}, "
