...
from quanthack.backtesting.deployment_profile_symbol_gate_refiner import (
    refine_deployment_profile_symbol_gates,
    write_deployment_profile_symbol_gate_refinement_csv,
    write_symbol_gated_profile_pack_json,
)
from quanthack.cli import deployment_profile_symbol_gate_refine
...
class DeploymentProfileSymbolGateRefinerTest(TestCase):
    def test_refiner_builds_candidates_from_weak_symbol_hours(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            output_path = root / "symbol_refine.csv"
            pack_output = root / "symbol_pack.json"
            _write_profile_pack(profile_path)
            _write_attribution_csv(attribution_path)

            result = refine_deployment_profile_symbol_gates(
                config=config,
                prices=data.prices,
                quotes=data.quotes,
                ...
                max_dropped_symbol_hours=2,
                include_walk_forward=False,
            )
            write_deployment_profile_symbol_gate_refinement_csv(result, output_path)
            two_drop = next(
                candidate
                for candidate in result.candidates
                if len(candidate.dropped_symbol_hours) == 2
            )
            write_symbol_gated_profile_pack_json(
            ...
            symbol_hours = dict(refined.symbol_allowed_utc_hours)
            csv_text = output_path.read_text(encoding="utf-8")
            json_text = pack_output.read_text(encoding="utf-8")

        self.assertEqual(len(result.weak_symbol_hours), 3)
        self.assertGreaterEqual(len(result.candidates), 3)
        self.assertNotIn(13, symbol_hours["EURUSD"])
        self.assertNotIn(13, symbol_hours["XAUUSD"])
        self.assertIn("dropped_symbol_hours", csv_text)
        self.assertIn('"recommended_slot": "symbol_refined"', json_text)

    def test_symbol_gate_refiner_cli_writes_artifacts(self) -> None:
        data = generate_synthetic_market_data(
            symbols=("EURUSD", "XAUUSD", "BTCUSD"),
            periods=48,
            interval_minutes=15,
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with redirect_stdout(stdout):
                deployment_profile_symbol_gate_refine.main(
                    [
                        "--config",
        ...
        self.assertIn("Deployment Profile Symbol-Gate Refinement", output)
        self.assertIn("Weak symbol hours", output)
        self.assertIn("symbol_refined", json_text)
        self.assertIn("dropped_symbol_hours", csv_text)
