...
from quanthack.backtesting.deployment_profile_dependency_replacement_refiner import (
    DEFAULT_DEPENDENCY_REPLACEMENT_SCALES,
    DEFAULT_REPLACEMENT_BASKET_SIZES,
    DEFAULT_REPLACEMENT_REFILL_FRACTIONS,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Try replacing fragile dependency-symbol exposure with capped diversified "
            "refill symbols selected from leave-one-symbol robustness evidence."
        )
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Fraction of freed multiplier budget to refill. Repeat to override defaults.",
    )
    parser.add_argument(
        "--basket-size",
        action="append",
        type=int,
        default=None,
        help="Number of replacement symbols to use. Repeat to override defaults.",
    )
    parser.add_argument("--dependency-threshold-pct", type=float, default=-0.003)
    parser.add_argument("--min-replacement-score-pct", type=float, default=0.0)
    parser.add_argument("--max-symbol-multiplier", type=float, default=1.0)
    parser.add_argument("--min-return-retention", type=float, default=0.75)
    parser.add_argument("--min-dependency-loss-reduction", type=float, default=0.50)
    ...
    parser.add_argument(
        "--output",
        default="outputs/research/deployment_profile_dependency_replacement.csv",
    )
    parser.add_argument(
        "--refined-pack-json",
        default="outputs/research/deployment_profile_dependency_replacement_pack.json",
    )
    parser.add_argument("--refined-slot", default="dependency_replacement")
...
def run(args: argparse.Namespace) -> None:
    ...
    result = refine_deployment_profile_dependency_replacement(
        ...
        dependency_scales=tuple(
            args.dependency_scale or DEFAULT_DEPENDENCY_REPLACEMENT_SCALES
        ),
        refill_fractions=tuple(
            args.refill_fraction or DEFAULT_REPLACEMENT_REFILL_FRACTIONS
        ),
        basket_sizes=tuple(args.basket_size or DEFAULT_REPLACEMENT_BASKET_SIZES),
        dependency_threshold_pct=args.dependency_threshold_pct,
        min_replacement_score_pct=args.min_replacement_score_pct,
        max_symbol_multiplier=args.max_symbol_multiplier,
    ...
    write_deployment_profile_dependency_replacement_csv(result, args.output)
    ...
    print("Deployment Profile Dependency Replacement")
    ...
    print(
        "  Replacement pool: "
        f"{', '.join(row.symbol for row in result.replacement_pool) or 'none'}"
    )
    ...
    for rank, candidate in enumerate(result.candidates[: max(args.limit, 0)], start=1):
        ...
        print(
            ...
            f"dep_scale={candidate.dependency_scale:.2f}, "
            f"refill={candidate.refill_fraction:.2f}, "
            f"basket={candidate.basket_size}, "
            ...
            f"freed={candidate.freed_multiplier:.3f}, "
            f"refilled={candidate.refilled_multiplier:.3f}, "
            f"promotion={'' if candidate.promotion is None else candidate.promotion.status}, "
            f"{dependency_text}"
            f"{wf_text}"
        )
        print(f"      replacements: {candidate.replacement_symbols_text or 'none'}")
