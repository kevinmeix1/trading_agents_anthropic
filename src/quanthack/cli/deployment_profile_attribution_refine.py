...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "that were weak in session attribution."
        )
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Weak-symbol multiplier scale to test. Repeat to override defaults.",
    )
    parser.add_argument("--weak-pnl-threshold-usd", type=float, default=0.0)
    parser.add_argument("--min-symbol-fills", type=int, default=1)
    ...
    parser.add_argument(
        "--output",
        default="outputs/research/deployment_profile_attribution_refinement.csv",
    )
    parser.add_argument(
        "--refined-pack-json",
        default="outputs/research/deployment_profile_refined_pack.json",
    )
    parser.add_argument("--refined-slot", default="refined")
...
def run(args: argparse.Namespace) -> None:
    ...
    result = refine_deployment_profile_from_attribution(
        ...
        weak_symbol_scales=tuple(args.weak_scale or DEFAULT_WEAK_SYMBOL_SCALES),
    ...
    write_deployment_profile_refinement_csv(result, args.output)
    ...
    print("Deployment Profile Attribution Refinement")
    print(f"  Base slot: {result.base_profile.slot}")
    print(f"  Base label: {result.base_profile.label}")
    print(f"  Weak symbols: {', '.join(row.symbol for row in result.weak_symbols) or 'none'}")
    ...
    for rank, candidate in enumerate(result.candidates[: max(args.limit, 0)], start=1):
        ...
        print(
            ...
            f"scale={candidate.weak_symbol_scale:.2f}, "
